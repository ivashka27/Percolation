...
cmake -S . -B build
cmake --build build
./build/percolation 20 30
```

Первый аргумент — размер решетки, второй аргумент — количество экспериментов.
Если аргументы не указаны, программа использует решетку 20x20 и 30 экспериментов.
